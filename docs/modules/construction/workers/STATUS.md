...
| Assign existing worker and create-and-assign | API/web implemented; mobile supports current-project create-and-assign | Service tests, web/mobile type-checks passed | Mobile existing-worker assignment is outside approved quick-flow scope |
| Update/end assignment and update current rate | API/web implemented | Service tests and web build passed | Attendance-aware elevated rule and effective-dated history depend on Attendance/Wages |
