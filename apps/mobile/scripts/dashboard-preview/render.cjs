...
const artifacts = path.resolve(process.env.DASHBOARD_PREVIEW_ARTIFACTS || "docs/tasks/artifacts/dashboard-redesign");
fs.mkdirSync(out, { recursive: true });
fs.mkdirSync(artifacts, { recursive: true });
(async () => {
  await esbuild.build({
    entryPoints: ["apps/mobile/scripts/dashboard-preview/preview.tsx"],
    bundle: true,
    ...
    plugins: [
      {
        ...
        setup(build) {
          ...
          build.onLoad({ filter: /.*/, namespace: "fixture" }, () => ({
            contents:
              "export const useLocalization=()=>({language:new URLSearchParams(location.search).get('lang')||'en'});",
            loader: "js",
          }));
  ...
  fs.writeFileSync(
    ...
    `<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>html,body{margin:0}*{box-sizing:border-box}${css}</style></head><body><div id="root"></div><script src="/preview.js"></script></body></html>`,
  );
      ...
      for (const width of [320, 375, 425]) {
        ...
        const overflow = await page.evaluate(() => ({
          ...
          offenders: [
            ...
            .filter(
              (x) =>
                x.rect.x < 0 || x.rect.right > innerWidth || x.rect.height < 50,
            ),
        }));
        await page.screenshot({
          path: path.join(artifacts, `dashboard-${width}-${language}.png`),
          fullPage: true,
        });
        const tabs = page.getByRole("tab");
        await tabs.nth(1).click();
        const actionsSelected = await tabs.nth(1).getAttribute("aria-selected");
        await tabs.nth(2).click();
        const attentionSelected = await tabs
          .nth(2)
          .getAttribute("aria-selected");
        if (language === "en")
          await page.screenshot({
            path: path.join(artifacts, `dashboard-${width}-attention.png`),
            fullPage: true,
          });
        report.push({
          language,
    ...
    if (
      report.some(
        (r) =>
          ...
          r.scroll > r.viewport ||
          r.offenders.length ||
          r.actionsSelected !== "true" ||
          r.attentionSelected !== "true",
      )
    )
      process.exitCode = 1;
  } finally {
