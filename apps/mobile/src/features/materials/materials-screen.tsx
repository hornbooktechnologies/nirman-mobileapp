...
export function MaterialsScreen() {
  ...
  return <NirmanScreenBackground footer={<CustomerTabBar activeKey="materials" />} scroll={false}>
    ...
    />
    <ListFilterSheet visible={filtersOpen} title={tCommon('listFilters.title')} description={t('filters.description')} clearLabel={tCommon('listFilters.clearAll')} applyLabel={tCommon('listFilters.apply')} onClear={() => { setDraftStatus(undefined); setDraftRequiredFrom(''); setDraftRequiredTo(''); setStatus(undefined); setRequiredFrom(''); setRequiredTo(''); setFiltersOpen(false); }} onApply={() => { if (draftDateRangeInvalid) return; setStatus(draftStatus); setRequiredFrom(draftRequiredFrom); setRequiredTo(draftRequiredTo); setFiltersOpen(false); }} onClose={() => setFiltersOpen(false)}><FormError message={draftDateRangeInvalid ? t('filters.dateRangeError') : ''} /><FilterGroup label={t('filters.status')}><FilterOption label={t('filters.allStatuses')} selected={!draftStatus} onPress={() => setDraftStatus(undefined)} />{MATERIAL_REQUEST_STATUSES.map((value) => <FilterOption key={value} label={t(`status.${value}`)} selected={draftStatus === value} onPress={() => setDraftStatus(value)} />)}</FilterGroup><FormField label={t('filters.from')}><DateInput accessibilityLabel={t('filters.fromA11y')} value={draftRequiredFrom} onChangeText={setDraftRequiredFrom} /></FormField><FormField label={t('filters.to')} error={draftDateRangeInvalid ? t('filters.dateRangeError') : undefined}><DateInput accessibilityLabel={t('filters.toA11y')} invalid={draftDateRangeInvalid} minimumDate={draftRequiredFrom ? dateValue(draftRequiredFrom) : undefined} value={draftRequiredTo} onChangeText={setDraftRequiredTo} /></FormField></ListFilterSheet>
...
function SettingsSheet({ visible, settings, organizationId, projectId, accessToken, onClose, onSaved }: { visible: boolean; settings: MaterialSettings | null; organizationId: string; projectId: string; accessToken: string; onClose: () => void; onSaved: (settings: MaterialSettings) => void }) {
  ...
  useEffect(() => { if (visible) { setMode(settings?.workflowMode ?? 'DIRECT'); setApprovers(settings?.approvalMembers?.filter((m) => m.delegated).map((m) => m.memberId) ?? []); setError(''); } }, [settings, visible]);
  async function save() { setWorking(true); setError(''); try { onSaved(await configureMaterialSettings(organizationId, projectId, accessToken, mode, settings?.canManageApprovers ? approvers : undefined, settings?.version ?? 0)); } catch (saveError) { setError(getLocalizedErrorMessage(saveError, t('errors.settingsFailed'))); } finally { setWorking(false); } }
  return <BottomSheet visible={visible} title={t('settings.title')} description={t('settings.description')} scroll showCloseButton={false} onClose={onClose} footer={<View style={styles.sheetFooter}><Button style={styles.footerButton} label={tCommon('actions.cancel')} variant="secondary" disabled={working} onPress={onClose} /><Button style={styles.footerButton} label={working ? t('loading.saving') : t('settings.save')} disabled={working} onPress={() => void save()} /></View>}><FormError message={error} />{error ? <Button variant="secondary" label={t('settings.refresh')} disabled={working} onPress={() => { setWorking(true); void fetchMaterialSettings(organizationId, projectId, accessToken).then(onSaved).catch((refreshError: unknown) => setError(getLocalizedErrorMessage(refreshError, t('errors.settingsFailed')))).finally(() => setWorking(false)); }} /> : null}{MATERIAL_WORKFLOW_MODES.map((value) => <FilterOption key={value} label={t(`workflow.${value}.label`)} selected={mode === value} onPress={() => setMode(value)} />)}<Card variant="blueprint"><AppText style={styles.workflowDescription}>{t(`workflow.${mode}.description`)}</AppText></Card><FilterGroup label={t('settings.approvers')}><AppText>{t('settings.approversHelp')}</AppText>{(settings?.approvalMembers ?? []).filter((m) => m.isOwner && m.canApprove).map((m) => <AppText key={m.memberId}>{m.name} · {t('settings.owner')}</AppText>)}{(settings?.approvalMembers ?? []).filter((m) => !m.isOwner).map((m) => settings?.canManageApprovers ? <FilterOption key={m.memberId} label={`${m.name} · ${m.roleName}`} selected={approvers.includes(m.memberId)} onPress={() => { if (!working) setApprovers((current) => current.includes(m.memberId) ? current.filter((id) => id !== m.memberId) : [...current, m.memberId]); }} /> : m.delegated ? <AppText key={m.memberId}>{m.name}</AppText> : null)}</FilterGroup></BottomSheet>;
}
