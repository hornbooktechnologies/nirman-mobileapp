...
export function SalesLeadScreen() {
  ...
  async function confirmBooking() {
    ...
    setError(null);
    try {
      const booking = await createBooking(organizationId, projectId, token, {
        idempotencyKey: bookingIdempotencyKey,
        leadId,
        ...(selectedUnit ? { unitId: selectedUnit.id } : {}),
        bookingDate: scheduleDate,
        ...(amount ? { bookingAmount: Number(amount) } : {}),
        ...(reference.trim() ? { bookingReference: reference.trim() } : {}),
      });
      setSheet(null);
      setBookingIdempotencyKey('');
      await load(true);
      Alert.alert(t('bookings.confirmedTitle'), t('bookings.confirmedDescription'), [
        { text: tCommon('progress.done') },
        {
          text: t('bookings.viewBooking'),
          onPress: () =>
            router.push({
  ...
  async function saveCustomerDetails() {
    ...
    try {
      ...
      setEditFormError(getLocalizedErrorMessage(cause, t('errors.save')));
    } finally {
      setWorking(false);
    }
  }
            ...
            {lead ? (
              <>
                    ...
                    <SalesDetailRows
                      rows={[
                        ...
                        },
                        {
                          ...
                          value: lead.budgetMin != null || lead.budgetMax != null ? `${lead.budgetMin == null ? '—' : formatInr(lead.budgetMin, language, { maximumFractionDigits: 0 })} – ${lead.budgetMax == null ? '—' : formatInr(lead.budgetMax, language, { maximumFractionDigits: 0 })}` : null,
                        },
                        {
                ...
                <View style={styles.actionList}>
                  ...
                  {permissions.includes('site-visits:manage') ? (
                    <SalesChoice
                  ...
                  {permissions.includes('inventory:interest') && lead.currentStage !== 'BOOKED' ? (
                    <SalesChoice
                  ...
                  {permissions.includes('leads:convert') && lead.currentStage !== 'BOOKED' ? (
                    <SalesChoice
