...
const remainingValue = (item: WageItem) =>
  Math.max(0, Number(item.netAmount) - Number(item.paidAmount));
...
export function WageBatchDetailScreen() {
  ...
  const canCancel = permissions.includes("wages:cancel");
  ...
  const selectedSettledByKharchi = selectedItem
    ? isSettledByKharchi(selectedItem)
    : false;
  ...
  const paymentError = !amount
    ...
    : !Number.isFinite(paymentValue) || paymentValue <= 0
      ...
      : paymentValue > selectedRemaining
        ? t("payment.exceedsDue", {
            amount: formatInr(selectedRemaining, language),
          })
        : "";
  ...
  async function saveAdjustment() {
    ...
    try {
      const signedAmount =
        adjustmentMode === "DEDUCT" ? -adjustmentValue : adjustmentValue;
  ...
  async function cancelBatch() {
    if (
      ...
      !session?.accessToken ||
      !detail ||
      cancellationReason.trim().length < 2
    )
      return;
    setIsBusy(true);
    try {
      const updated = await cancelWageBatch(
        organizationId,
        projectId,
        detail.id,
      ...
      setSelectedItemId(null);
      Alert.alert(
        t("cancellation.successTitle"),
        t("cancellation.successMessage"),
      );
    } catch (cancelFailure) {
      Alert.alert(
        t("cancellation.failedTitle"),
        getLocalizedErrorMessage(
          cancelFailure,
          t("cancellation.failedMessage"),
        ),
      );
    } finally {
      setIsBusy(false);
  ...
  const totalDue = detail
    ? Math.max(
        0,
        Number(detail.totals.netAmount) - Number(detail.totals.paidAmount),
      )
    : 0;
  const isCancelled = detail?.status === "CANCELLED";
  const batchHasPayments = Boolean(
    detail &&
    (detail.payments.length > 0 || Number(detail.totals.paidAmount) > 0),
  );
  const canManageItems = !isCancelled && (canPay || canUpdate);
  const header = (
      ...
      {detail ? (
        <Card style={styles.summaryCard}>
          <View style={styles.summaryTopRow}>
            <View style={styles.sectionHeading}>
              <AppText style={styles.period} weight={700}>
                {formatDate(dateValue(detail.periodStart), language)} –{" "}
                {formatDate(dateValue(detail.periodEnd), language)}
              </AppText>
              <AppText style={styles.muted} weight={500}>
                {t("detail.workerCount", { count: detail.items.length })}
              </AppText>
            </View>
            <StatusBadge
              label={t(`batchStatus.${detail.status}`)}
          ...
          <View style={styles.totalsRow}>
            <Total
              label={t("detail.net")}
              value={formatInr(Number(detail.totals.netAmount), language)}
            />
            <Total
              label={t("detail.paid")}
              value={formatInr(Number(detail.totals.paidAmount), language)}
            />
            <Total
              emphasis
              label={t("detail.due")}
              value={formatInr(totalDue, language)}
            />
          </View>
          {canExport ? (
            <Button
          ...
          {isCancelled ? (
            <AppText style={styles.infoMessage} weight={600}>
              {t("cancellation.cancelledReason", {
                reason:
                  detail.cancellationReason ??
                  t("cancellation.reasonUnavailable"),
              })}
            </AppText>
          ) : canCancel ? (
            <>
              <Button
                label={t("cancellation.action")}
                leadingIcon="close-circle-outline"
                variant="danger"
                disabled={isBusy || batchHasPayments}
                onPress={() => setIsCancelOpen(true)}
              />
              {batchHasPayments ? (
                <AppText style={styles.muted}>
                  {t("cancellation.paymentBlocked")}
                </AppText>
              ) : null}
            </>
          ) : null}
      ...
      {detail ? (
        <View style={styles.sectionHeading}>
          <AppText style={styles.sectionTitle} weight={700}>
            {t("detail.workersTitle")}
          </AppText>
          <AppText style={styles.muted} weight={500}>
            {t("detail.workersDescription")}
          </AppText>
        </View>
      ) : null}
      {error ? (
        <EmptyState
          title={t("errors.batchTitle")}
          description={error}
    ...
    <>
          ...
          renderItem={({ item }) => {
            ...
            return (
              <OperationalEntityCard
                ...
                footerTrailing={
                  canManageItems ? (
                    <AppText style={styles.manage} weight={700}>
                      {t("worker.manage")}
                    </AppText>
                  ) : undefined
                }
                ...
                onPress={canManageItems ? () => openWorker(item) : undefined}
              />
            );
          }}
      ...
      {selectedItem ? (
        ...
        >
          <OperationalEntityCard
            ...
            supporting={t("worker.paidAmount", {
              amount: formatInr(Number(selectedItem.paidAmount), language),
            })}
              ...
              <Badge
                label={
                  selectedSettledByKharchi
                    ? t("paymentStatus.KHARCHI_SETTLED")
                    : t(`paymentStatus.${selectedItem.paymentStatus}`)
                }
                tone={
                  selectedSettledByKharchi
                    ? "info"
            ...
            details={
              <View style={styles.calculationDetails}>
                <CalculationLine
                  ...
                  value={t("worker.perDayAmount", {
                    amount: formatInr(Number(selectedItem.dailyRate), language),
                  })}
                />
                <CalculationLine
                  label={t("worker.attendanceLabel")}
                  value={t("worker.attendance", {
                    present: selectedItem.presentDays,
                    half: selectedItem.halfDays,
                    absent: selectedItem.absentDays,
                  })}
                />
                <CalculationLine
                  label={t("worker.grossWage")}
                  value={formatInr(Number(selectedItem.grossAmount), language)}
                />
                <CalculationLine
                  label={t("worker.kharchiDeduction")}
                  value={`−${formatInr(Number(selectedItem.kharchiDeduction), language)}`}
                />
                <CalculationLine
                  label={t("worker.adjustment")}
                  value={formatInr(
                    Number(selectedItem.adjustmentAmount),
                    language,
                  )}
                />
              </View>
          ...
          {canPay ? (
            <View style={styles.sheetSection}>
              ...
              {selectedRemaining > 0 ? (
                <>
                  <View style={styles.formRow}>
                    ...
                    >
                      <DateInput
                        allowClear={false}
                        accessibilityLabel={t("payment.selectDate")}
                        value={paymentDate}
                        onChangeText={(value) => value && setPaymentDate(value)}
                      />
                    </FormField>
                  </View>
                  <FormField label={t("payment.method")} required>
                    <View style={styles.methodRow}>
                      {WAGE_PAYMENT_METHODS.map((method) => (
                        ...
                        >
                          <AppText style={styles.methodText} weight={700}>
                  ...
                  <Button
                    label={
                      isBusy ? t("payment.recording") : t("payment.action")
                    }
                ...
                <AppText style={styles.infoMessage} weight={600}>
                  {t("payment.settledByKharchi", {
                    amount: formatInr(
                      Number(selectedItem.kharchiDeduction),
                      language,
                    ),
                  })}
                </AppText>
              ) : selectedItem.paymentStatus === "PAID" ? (
                <AppText style={styles.successMessage} weight={600}>
                  {t("payment.alreadyPaid")}
                </AppText>
              ) : (
                <AppText style={styles.infoMessage} weight={600}>
                  {t("payment.noPaymentDue")}
                </AppText>
              )}
            </View>
          ) : null}
          ...
          {canUpdate ? (
            <View style={styles.sheetSection}>
              ...
              <View style={styles.modeRow}>
                {(["ADD", "DEDUCT"] as const).map((mode) => (
                  <Pressable
                    ...
                    style={({ pressed }) => [
                      styles.modeButton,
                      adjustmentMode === mode && styles.modeSelected,
                      pressed && styles.pressed,
                    ]}
                    onPress={() => setAdjustmentMode(mode)}
                  >
                    <AppText style={styles.modeText} weight={700}>
                      {t(`adjustment.mode.${mode}`)}
                    </AppText>
                  </Pressable>
                ))}
              </View>
              <FormField
                ...
                helperText={t(
                  adjustmentMode === "ADD"
                    ? "adjustment.addHelper"
                    : "adjustment.deductHelper",
                )}
              >
                <Input
              ...
              <FormError
                message={
                  ...
                  adjustmentValue > Number(selectedItem.netAmount)
                    ? t("adjustment.largeDeduction")
                    : undefined
                }
              />
              <Button
                ...
                variant="brand"
                disabled={
                  !adjustmentAmount || Boolean(adjustmentError) || isBusy
                }
      ...
      >
        <FormField
          ...
          required
          helperText={t("cancellation.reasonHelper")}
        >
          <Input
        ...
        <Button
          label={
            isBusy ? t("cancellation.cancelling") : t("cancellation.confirm")
          }
          leadingIcon="close-circle-outline"
          variant="danger"
          disabled={isBusy || cancellationReason.trim().length < 2}
          onPress={() => void cancelBatch()}
        />
        <Button
...
function Total({
  ...
  return (
    ...
    >
      <AppText style={styles.totalLabel} weight={600}>
        {label}
      </AppText>
      <AppText
        style={[styles.totalValue, emphasis && styles.totalEmphasis]}
        weight={700}
      >
        {value}
      </AppText>
...
function CalculationLine({ label, value }: { label: string; value: string }) {
  return (
    <View
      accessible
      accessibilityLabel={`${label}: ${value}`}
      style={styles.calculationLine}
    >
      <AppText style={styles.calculationLabel} weight={500}>
        {label}
      </AppText>
      <AppText style={styles.calculationValue} weight={700}>
        {value}
      </AppText>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  successMessage: {
    ...
    borderColor: mobileTheme.color.status.success.border,
    borderRadius: mobileTheme.radius.md,
    borderWidth: 1,
  ...
  infoMessage: {
    ...
    borderRadius: mobileTheme.radius.md,
    borderWidth: 1,
    color: mobileTheme.color.status.info.foreground,
    padding: mobileTheme.spacing[3],
  },
