...
import { useUnsavedAttendanceForm } from "@/features/attendance/use-unsaved-attendance-form";
import { validMonth, workToday } from "@/features/attendance/date-utils";
...
export function WorkCalendarPage() {
  ...
  return <CalendarContent key={`${user?.id}:${activeOrganizationId}:${accessKey}:${params.get("projectId")}:${params.get("month")}`} />;
}
...
function CalendarContent() {
  ...
  const access = useProjectAccess(activeOrganizationId);
  const organization = useOrganizationCalendar(activeOrganizationId, hasPermission("work-calendar:read"));
  const projects = useMemo(() => (access.data?.projects ?? []).filter(project => project.permissions.includes("work-calendar:read")), [access.data?.projects]);
  const requestedProjectId = searchParams.get("projectId") ?? "";
  const selectedProject = requestedProjectId ? projects.find((item) => item.id === requestedProjectId) ?? null : projects.find(item => item.isDefault) ?? projects[0] ?? null;
  const projectId = selectedProject?.id ?? "";
  const timezone = organization.data?.timezone ?? activeOrganizationTimezone;
  const rawMonth = searchParams.get("month") ?? (timezone ? workToday(timezone).slice(0, 7) : "");
  const month = validMonth(rawMonth) ? rawMonth : (timezone ? workToday(timezone).slice(0, 7) : "2000-01");
  const range = useMemo(() => monthRange(month), [month]);
  const projectCalendar = useProjectCalendar(activeOrganizationId, projectId, range.startDate, range.endDate, Boolean(selectedProject && timezone) && access.isSuccess);
  ...
  const canUpdateProject = access.isSuccess && selectedPermissions.includes("work-calendar:update-project");
  const selectedDate = searchParams.get("selectedDate") ?? "";
  const selectedDay = projectCalendar.data?.days.find(day => day.date === selectedDate) ?? projectCalendar.data?.days[0];
  ...
  const [initialForm, setInitialForm] = useState<OverrideForm>(emptyOverride);
  const [weekDirty, setWeekDirty] = useState(false);
  const formDirty = Boolean(dialog) && JSON.stringify(form) !== JSON.stringify(initialForm);
  const confirmDiscard = useUnsavedAttendanceForm(formDirty || weekDirty);
  ...
  function replaceQuery(updates: Record<string, string | null>) { if (pending || ((updates.projectId !== undefined || updates.month !== undefined) && !confirmDiscard())) return; const next = new URLSearchParams(searchParams.toString()); Object.entries(updates).forEach(([key, value]) => value ? next.set(key, value) : next.delete(key)); router.replace(`/work-calendar?${next}`); }


  function openOverride(scope: CalendarScope, item: WorkCalendarOverride | null = null) {
    const date = scope === "PROJECT" ? selectedDay?.date ?? range.startDate : range.startDate;
    const next = item ? { startDate: item.startDate, endDate: item.endDate, dayType: item.dayType, name: item.name, reason: item.reason ?? "" } : { ...emptyOverride, startDate: date, endDate: date };
    setDialog({ scope, item }); setForm(next); setInitialForm(next); setFormError(""); setConfirmRemove(false);
  }
  function closeOverride() { if (!pending && (!formDirty || window.confirm("Discard changes to this override?"))) setDialog(null); }
  async function submit(event: FormEvent) { event.preventDefault(); if (!dialog || pending) return; if (dialog.scope === "PROJECT" ? !canUpdateProject : !canUpdateOrganization) { setFormError("Your permission changed. Refresh access before saving."); return; } if (!form.startDate || !form.endDate || !form.name.trim()) { setFormError("Start date, end date, and name are required."); document.getElementById("override-name")?.focus(); return; } if (form.endDate < form.startDate) { setFormError("End date must be on or after the start date."); return; } setFormError(""); const input = { startDate: form.startDate, endDate: form.endDate, dayType: form.dayType, name: form.name.trim(), reason: form.reason.trim() || null }; try { if (dialog.item) await updateOverride.mutateAsync({ scope: dialog.scope, overrideId: dialog.item.id, input }); else await createOverride.mutateAsync({ scope: dialog.scope, input }); setDialog(null); setSuccess(`${dialog.scope === "PROJECT" ? "Project" : "Organization"} calendar override saved.`); } catch (error) { setFormError(message(error)); } }
  async function remove() { if (!dialog?.item || pending) return; if (dialog.scope === "PROJECT" ? !canUpdateProject : !canUpdateOrganization) { setFormError("Your permission changed. Refresh access before removing this override."); return; } setFormError(""); try { await removeOverride.mutateAsync({ scope: dialog.scope, overrideId: dialog.item.id }); setDialog(null); setSuccess("Calendar override removed."); } catch (error) { setFormError(message(error)); setConfirmRemove(false); } }
  ...
  return <div className="space-y-4 pb-8 text-base [&_button]:min-h-11 [&_button]:text-sm [&_input]:min-h-11 [&_select]:min-h-11">
    ...
    <Card padding="compact"><div className="grid gap-3 md:grid-cols-2"><div><p className="text-xs font-bold uppercase tracking-wide text-sub">Organization calendar</p><p className="mt-1 font-semibold">Normal working week and organization-wide dates</p></div><label className="grid gap-1.5 font-semibold">Project calendar<Select className="text-base" value={projectId} disabled={!projects.length} onChange={(event) => replaceQuery({ projectId: event.target.value })}>{projects.length ? projects.map((item) => <option key={item.id} value={item.id}>{item.name}{item.projectCode ? ` · ${item.projectCode}` : ""}</option>) : <option value="">No accessible projects</option>}</Select></label></div></Card>

    {hasPermission("work-calendar:read") ? organization.isLoading ? <LoadingState label="Loading organization calendar" /> : organization.isError ? <NotificationBanner variant="danger" title="Organization calendar could not be loaded" description={message(organization.error)} action={<Button variant="outline" onClick={() => organization.refetch()}>Retry</Button>} /> : <WeeklySetup key={`${organization.data?.updatedAt ?? "unconfigured"}-${organization.data?.timezone ?? "none"}`} organizationId={activeOrganizationId} calendar={organization.data} canUpdate={canUpdateOrganization} onDirtyChange={setWeekDirty} onSaved={() => { setWeekDirty(false); setSuccess("Normal working week saved."); void refreshUser(); }} /> : projectCalendar.data ? <WeeklySetup key={projectId} organizationId={activeOrganizationId} calendar={{ ...projectCalendar.data, updatedAt: null }} canUpdate={false} onSaved={() => undefined} /> : null}

    {(hasPermission("work-calendar:read") ? organization.isSuccess : projectCalendar.isSuccess) ? <OverrideSection title="Organization overrides" description="Dates that apply to every project unless a project override takes precedence." items={hasPermission("work-calendar:read") ? organizationOverrides : projectCalendar.data?.organizationOverrides ?? []} canUpdate={canUpdateOrganization} onAdd={() => openOverride("ORGANIZATION")} onEdit={(item) => openOverride("ORGANIZATION", item)} /> : null}
    ...
    {!projects.length ? <EmptyState title="No accessible projects" description="You can review the Organization calendar, but a Project calendar requires Project access." /> : !selectedProject ? <LoadingState label="Selecting an accessible project" /> : <>
      <Card padding="compact"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-semibold">Effective project calendar</h2><p className="mt-1 text-sub">Project override &gt; Organization override &gt; Weekly pattern</p></div><div className="flex items-center gap-2"><Button variant="outline" aria-label="Previous month" onClick={() => replaceQuery({ month: shiftMonth(month, -1) })}><ChevronLeft size={16} aria-hidden="true" /></Button><Input className="w-40 text-base" aria-label="Calendar month" type="month" value={month} onChange={(event) => replaceQuery({ month: event.target.value })} /><Button variant="outline" aria-label="Next month" onClick={() => replaceQuery({ month: shiftMonth(month, 1) })}><ChevronRight size={16} aria-hidden="true" /></Button></div></div></Card>
      {projectCalendar.isLoading ? <LoadingState label="Loading effective project calendar" /> : projectCalendar.isError ? <NotificationBanner variant="danger" title="Project calendar could not be loaded" description={message(projectCalendar.error)} action={<Button variant="outline" onClick={() => projectCalendar.refetch()}>Retry</Button>} /> : !projectCalendar.data?.configured ? <NotificationBanner variant="warning" title="Calendar not configured" description="The Organization working week must be configured before working dates can be determined." /> : <><NotificationBanner variant="info" title={projectOverrides.length ? "Project dates take precedence" : "Using Organization calendar"} description={projectOverrides.length ? "Project overrides are clearly labelled in the month below." : "No Project override changes this month."} /><MonthGrid month={month} days={projectCalendar.data.days} selectedDate={selectedDay?.date ?? ""} onSelect={date => replaceQuery({ selectedDate: date })} />
      {selectedDay ? <Card><div className="flex flex-wrap items-center justify-between gap-3"><h3 className="font-semibold">{selectedDay.date}</h3><StatusBadge tone={dayTones[selectedDay.dayType]}>{dayLabels[selectedDay.dayType]}</StatusBadge></div><p className="mt-2">{sourceLabels[selectedDay.source]}</p><p className="mt-2 break-words text-sub">{selectedDay.override?.name ?? "Using the normal weekly pattern."}</p>{selectedDay.override?.reason ? <p className="mt-2 whitespace-pre-wrap break-words">{selectedDay.override.reason}</p> : null}{canUpdateProject ? <Button className="mt-4" onClick={() => openOverride("PROJECT", selectedDay.source === "PROJECT_OVERRIDE" ? selectedDay.override : null)}>{selectedDay.source === "PROJECT_OVERRIDE" ? "Edit project override" : "Add project override"}</Button> : null}</Card> : null}</>}
      {projectCalendar.isSuccess ? <OverrideSection title="Project overrides" description="Site-specific closures or special working dates for the selected project." items={projectOverrides} canUpdate={canUpdateProject} onAdd={() => openOverride("PROJECT")} onEdit={(item) => openOverride("PROJECT", item)} /> : null}
    </>}

    <Dialog open={Boolean(dialog)} onOpenChange={open => { if (!open) closeOverride(); }} title={`${dialog?.item ? "Edit" : "Add"} ${dialog?.scope === "PROJECT" ? "Project" : "Organization"} override`} description="Use one date range for a closure or special working period." footer={confirmRemove ? <><Button variant="outline" onClick={() => setConfirmRemove(false)} disabled={pending}>Keep override</Button><Button variant="danger" onClick={remove} disabled={pending}>{pending ? "Removing" : "Confirm removal"}</Button></> : <><Button variant="outline" onClick={closeOverride} disabled={pending}>Cancel</Button>{dialog?.item ? <Button variant="danger" onClick={() => setConfirmRemove(true)} disabled={pending}>Remove</Button> : null}<Button type="submit" form="calendar-override-form" disabled={pending}>{pending ? "Saving" : dialog?.item ? "Update override" : "Save override"}</Button></>}>
      <form id="calendar-override-form" className="space-y-4 text-base [&_button]:min-h-11" onSubmit={submit}><fieldset disabled={pending} className="space-y-4">{formError ? <NotificationBanner variant="danger" title="Override was not saved" description={formError} /> : null}<div className="grid gap-3 sm:grid-cols-2"><label className="grid gap-1.5 font-semibold">Start date *<Input required className="text-base" type="date" value={form.startDate} onChange={(event) => setForm((value) => ({ ...value, startDate: event.target.value }))} /></label><label className="grid gap-1.5 font-semibold">End date *<Input required className="text-base" type="date" min={form.startDate} value={form.endDate} onChange={(event) => setForm((value) => ({ ...value, endDate: event.target.value }))} /></label></div><label className="grid gap-1.5 font-semibold">Day type *<Select required className="text-base" value={form.dayType} onChange={(event) => setForm((value) => ({ ...value, dayType: event.target.value as WorkCalendarDayType }))}><option value="NON_WORKING">Non-working</option><option value="SPECIAL_WORKING">Special working</option></Select></label><label className="grid gap-1.5 font-semibold">Name *<Input id="override-name" required className="text-base" maxLength={160} value={form.name} onChange={(event) => setForm((value) => ({ ...value, name: event.target.value }))} /></label><label className="grid gap-1.5 font-semibold">Reason<Textarea className="text-base" maxLength={2000} value={form.reason} onChange={(event) => setForm((value) => ({ ...value, reason: event.target.value }))} /></label>{confirmRemove ? <NotificationBanner variant="warning" title="Remove this override?" description="The effective calendar will fall back to the next applicable source." /> : null}</fieldset></form>
    </Dialog>
  </div>;
}

function WeeklySetup({ organizationId, calendar, canUpdate, onSaved, onDirtyChange }: { organizationId: string; calendar?: { configured: boolean; timezone: string; workingWeek: WorkingWeek | null; updatedAt: string | null }; canUpdate: boolean; onSaved: () => void; onDirtyChange?: (dirty: boolean) => void }) {
  ...
  useEffect(() => {
    onDirtyChange?.(timezone !== (calendar?.timezone ?? "") || JSON.stringify(workingWeek) !== JSON.stringify(calendar?.workingWeek ?? emptyWorkingWeek));
  }, [timezone, workingWeek, calendar?.timezone, calendar?.workingWeek, onDirtyChange]);

  async function saveWeek(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!canUpdate || updateCalendar.isPending) return;
    ...
    try { new Intl.DateTimeFormat("en-IN", { timeZone: normalizedTimezone }).format(); }
    catch { setTimezoneError("Enter a valid IANA timezone, such as Asia/Kolkata."); document.getElementById("weekly-timezone")?.focus(); return; }
...
function MonthGrid({ month, days, selectedDate, onSelect }: { month: string; days: EffectiveWorkCalendarDay[]; selectedDate: string; onSelect: (date: string) => void }) { const [year, value] = month.split("-").map(Number); const offset = (new Date(year, value - 1, 1).getDay() + 6) % 7; return <Card padding="compact"><div className="space-y-2 sm:hidden">{days.map((day) => <button type="button" key={day.date} aria-pressed={day.date === selectedDate} onClick={() => onSelect(day.date)} className="flex w-full flex-wrap items-center justify-between gap-3 rounded-inner border border-hairline p-3 text-left focus-visible:outline focus-visible:outline-2 focus-visible:outline-lime aria-pressed:bg-sunken"><div><p className="font-semibold tabular-nums">{day.date}</p><p className="mt-1 text-sm text-sub">{sourceLabels[day.source]}{day.override ? ` · ${day.override.name}` : ""}</p></div><StatusBadge className="shrink-0" tone={dayTones[day.dayType]}>{dayLabels[day.dayType]}</StatusBadge></button>)}</div><div className="hidden sm:block"><div className="grid grid-cols-7 gap-1 text-center text-xs font-bold uppercase tracking-wide text-sub">{["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day) => <span key={day}>{day}</span>)}</div><div className="mt-2 grid grid-cols-7 gap-1">{Array.from({ length: offset }, (_, index) => <span key={`empty-${index}`} />)}{days.map((day) => <DayCell key={day.date} day={day} selected={day.date === selectedDate} onSelect={() => onSelect(day.date)} />)}</div></div><div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-sm text-sub"><StatusBadge tone="active">Working</StatusBadge><StatusBadge tone="neutral">Non-working</StatusBadge><StatusBadge tone="warning">Special working</StatusBadge><span>Sources: Project override</span><span>Organization override</span><span>Weekly pattern</span></div></Card>; }
function DayCell({ day, selected, onSelect }: { day: EffectiveWorkCalendarDay; selected: boolean; onSelect: () => void }) { return <button type="button" aria-label={`${day.date}: ${dayLabels[day.dayType]}, ${sourceLabels[day.source]}`} aria-pressed={selected} onClick={onSelect} className="min-h-24 min-w-0 rounded-inner border border-hairline p-2 text-left focus-visible:outline focus-visible:outline-2 focus-visible:outline-lime aria-pressed:bg-sunken"><p className="font-semibold tabular-nums">{Number(day.date.slice(-2))}</p><StatusBadge className="mt-2 max-w-full whitespace-normal text-left" tone={dayTones[day.dayType]}>{dayLabels[day.dayType]}</StatusBadge><p className="mt-1 text-xs leading-4 text-sub">{sourceLabels[day.source]}</p>{day.override ? <p className="mt-1 truncate text-xs font-medium" title={day.override.name}>{day.override.name}</p> : null}</button>; }
