...
export function GalleryDetail({ entry, context: c, close, changed }: { entry: GalleryEntry; context: GalleryContext; close: () => void; changed: (message?: string) => void }) {
  ...
  return <Dialog open title="Photo details" className="max-w-3xl" onOpenChange={() => { if (!busy) close(); }}>
    <div className="space-y-4 text-base"><PrivateImage org={c.org} project={c.project} id={entry.id} alt={entry.caption || `${label(entry.category)} photo`} full />
      <div className="flex flex-wrap items-center justify-between gap-2"><p className="font-semibold">{label(entry.category)}{entry.stage ? ` · ${label(entry.stage)}` : ""}</p><StatusBadge tone={entry.status === "APPROVED" ? "success" : entry.status === "PENDING" ? "warning" : "neutral"}>{label(entry.status)}</StatusBadge></div>
      <p className="whitespace-pre-wrap break-words">{entry.caption || "No caption provided for this photo."}</p>
      {entry.stage ? <p className="text-sm text-sub">Stage tag only; no individual Progress update is linked to this photo.</p> : null}
      <dl className="grid gap-4 sm:grid-cols-2">{[["Captured", `${dateTime(entry.capturedAt)} IST`], ["Uploaded by", entry.uploadedBy], ["Uploaded", `${dateTime(entry.createdAt)} IST`]].map(([name, value]) => <div key={name}><dt className="text-sm text-sub">{name}</dt><dd className="break-words">{value}</dd></div>)}</dl>
      {entry.rejectionReason && <p className="whitespace-pre-wrap break-words">Rejection reason: {entry.rejectionReason}</p>}
      <details className="rounded-inner border border-hairline p-3 text-sm"><summary className="cursor-pointer font-semibold">File and review metadata</summary><dl className="mt-3 grid gap-3 sm:grid-cols-2">{[["File", `${entry.mimeType} · ${(entry.byteSize / 1024 / 1024).toFixed(2)} MiB`], ["Dimensions", entry.width && entry.height ? `${entry.width} × ${entry.height}` : "Not recorded"], ["Version", String(entry.version)], ["Reviewed by", entry.reviewedBy || "Not reviewed"], ["Reviewed", entry.reviewedAt ? dateTime(entry.reviewedAt) : "Not reviewed"]].map(([name, value]) => <div key={name}><dt className="text-sub">{name}</dt><dd className="break-words">{value}</dd></div>)}</dl></details>
