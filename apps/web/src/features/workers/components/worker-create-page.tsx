...
export function WorkerCreatePage() {
  return <WorkerWorkspace permission="workers:create">{organizationId => <WorkerCreate organizationId={organizationId} />}</WorkerWorkspace>;
}
...
function WorkerCreate({ organizationId }: { organizationId: string }) {
  ...
  const projects = access.data?.projects.filter(project => project.status !== "ARCHIVED" && project.permissions.includes("workers:create") && project.permissions.includes("workers:assign-project")) ?? [];
      ...
      <div className="space-y-4">
        ...
        {!organizationId ? (
          ...
          <Card className="space-y-4">
            {access.isPending ? <LoadingState label="Loading available projects" /> : access.isError ? <div role="alert"><p>{access.error.message}</p><Button onClick={() => void access.refetch()}>Retry projects</Button></div> : <label className="grid gap-1 text-base">Initial project<Select value={projectId} disabled={createWorker.isPending || formBusy} onChange={event => { if (window.confirm("Changing project resets this worker form. Continue?")) setProjectId(event.target.value); }}><option value="">Create without an assignment</option>{projects.map(project => <option key={project.id} value={project.id}>{project.name}</option>)}</Select></label>}
            {!validProject ? <p role="alert">This project is unavailable or you do not have permission to create and assign workers there.</p> : null}
            {validProject && !access.isPending && !access.isError ? <WorkerForm
