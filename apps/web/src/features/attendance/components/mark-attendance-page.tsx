...
function DailyAttendance() {
  ...
  const { activeOrganizationId, activeOrganizationTimezone, refreshUser } =
    useAuth();
  const access = useProjectAccess(activeOrganizationId);
  ...
  const date =
    ...
    (activeOrganizationTimezone ? workToday(activeOrganizationTimezone) : "");
  const rawSearch = searchParams.get("markSearch") ?? "";
  const [search, setSearch] = useState(rawSearch);
  const [debouncedSearch, setDebouncedSearch] = useState(search);
  const page = Math.max(
    1,
    Math.floor(Number(searchParams.get("markPage")) || 1),
  );
  ...
  const [editingRow, setEditingRow] = useState<AttendanceSummaryRow | null>(
    null,
  );
  const [restoreRow, setRestoreRow] = useState<AttendanceSummaryRow | null>(
    null,
  );
  ...
  const can = (permission: string) =>
    access.isSuccess && permissions.includes(permission);
  ...
  const updateException = useUpdateAttendanceException(
    activeOrganizationId,
    projectId,
  );
  const removeException = useRemoveAttendanceException(
    activeOrganizationId,
    projectId,
  );
  ...
  function replaceQuery(updates: Record<string, string>) {
    if (
      ...
      (dirty && !window.confirm("Discard changes to this absence?"))
    )
      return;
  ...
  useEffect(() => {
    ...
    const next = new URLSearchParams(searchParams.toString());
    next.set("markSearch", debouncedSearch);
    next.set("markPage", "1");
  ...
  [
    ...
    "workerId",
    "workerName",
  ].forEach((key) => backParams.delete(key));
  if (projectId) backParams.set("projectId", projectId);
  const backHref =
    requestedReturn &&
    (requestedReturn.startsWith("/attendance?") ||
      /^\/workers\/[0-9a-f-]+\?/i.test(requestedReturn))
      ? requestedReturn
      : `/attendance?${backParams}`;
  ...
  const dirty = Boolean(
    editingRow &&
    (draft.duration !== (originalDraft?.duration ?? "FULL_DAY") ||
  ...
  const handleExceptionOpenChange = useCallback(
    (open: boolean) => {
      if (
        !open &&
        !isSaving &&
    ...
    [isSaving, dirty, setEditingRow, setFormError],
  );

  const handleRestoreOpenChange = useCallback(
    (open: boolean) => {
      if (!open && !removeException.isPending) setRestoreRow(null);
    },
    [removeException.isPending, setRestoreRow],
  );
  ...
  async function saveException() {
    ...
    if (
      !canRead ||
      !(editingRow.selectedDate?.exception ? canUpdate : canMark)
    ) {
      setFormError(
        "Your attendance permission changed. Refresh access before saving.",
      );
      return;
    }
    setFormError("");
    ...
    try {
      ...
      setEditingRow(null);
      setSuccess(
        `${editingRow.worker.name} marked ${draft.duration === "FULL_DAY" ? "absent" : "half day"}.`,
      );
    } catch (error) {
  ...
  async function confirmRestore() {
    ...
    if (!restoreRow || !exception || removeException.isPending || !canUpdate)
      return;
    try {
      await removeException.mutateAsync(exception.id);
      setSuccess(
        `Absence removed for ${restoreRow.worker.name}. Attendance has been refreshed.`,
      );
      setRestoreRow(null);
    } catch (error) {
      setFormError(errorMessage(error));
  ...
  const nonWorking =
    rows.length > 0 &&
    rows.every((row) => row.selectedDate?.state === "NON_WORKING");
  ...
  return (
    <div className="space-y-4 pb-8 text-sm">
      <PageHeader
        ...
        onBack={() => {
          if (confirmDiscard()) router.push(backHref);
        }}
      />
      ...
      {selectedProject ? (
        <AttendanceNavigation
          ...
          calendarDate={date}
          summaryHref={backHref}
          markHref={`/attendance/mark?${searchParams}`}
          current="mark"
          onNavigate={confirmDiscard}
        />
      ) : null}
      {selectedProject?.status === "ARCHIVED" ? (
        <NotificationBanner
          ...
          description="You are viewing attendance for an archived project. Changes remain subject to API validation."
        />
      ) : null}
      <p className="text-sm text-sub">
        Working timezone: {activeOrganizationTimezone ?? "Unavailable"}.
        Eligibility uses the effective primary-project allocation for this date.
      </p>
      {success ? (
      ...
      {!activeOrganizationId ? (
        <EmptyState
          ...
          description="Select an organization before marking attendance."
        />
      ) : access.isLoading ? (
        ...
        <EmptyState
          title={
            requestedProjectId
              ? "Project attendance access required"
              : "No accessible projects"
          }
          description="Select a project with effective attendance permission."
          action={
            <Button onClick={() => router.replace("/attendance/mark")}>
        ...
        <NotificationBanner
          ...
          description="Attendance read permission is required."
        />
      ) : (
        <>
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_190px_minmax(0,1fr)] md:items-end">
              ...
              <label className="grid gap-1.5 font-semibold">
                ...
                <Input
                  ...
                  value={date}
                  onChange={(event) =>
                    replaceQuery({ date: event.target.value })
                  }
                />
              </label>
              <label className="grid gap-1.5 font-semibold">
          ...
          <Checkbox
            ...
            onChange={(event) =>
              replaceQuery({ markExceptionsOnly: String(event.target.checked) })
            }
          />
          {!canMark && !canUpdate ? (
            <NotificationBanner
              variant="info"
              title="Read-only attendance"
              description="You can review this roster but cannot change absences."
            />
          ) : null}
          {summary.data && !summary.isError ? (
            <section
              aria-label="Daily attendance totals"
              className="grid grid-cols-2 gap-3 lg:grid-cols-4"
            >
              {[
                ["Workers", summary.data.totals.workers],
                [
                  "Expected worker-days",
                  summary.data.totals.expectedWorkingDays,
                ],
                ["Present worker-days", summary.data.totals.presentDays],
                ["Absent worker-days", summary.data.totals.absentDays],
              ].map(([label, value]) => (
          ...
          {nonWorking ? (
            <NotificationBanner
              ...
              description="Absence cannot be recorded because this date is non-working in the effective Project calendar."
            />
          ) : null}
          ...
          {!validDate(date) ? (
            ...
            <EmptyState
              title="No workers for this date"
              description={
                search || exceptionsOnly
                  ? "No workers match your filters. Clear search or the exceptions-only filter."
                  : "No primary worker assignments cover the selected date."
              }
            />
          ) : (
            <DailyRoster
          ...
          {summary.data && page > Math.max(1, summary.data.meta.totalPages) ? (
            <NotificationBanner
              ...
              action={
                <Button onClick={() => replaceQuery({ markPage: "1" })}>
          ...
          {summary.data && summary.data.meta.totalPages > 1 ? (
            <div className="flex items-center justify-between gap-3">
              <p className="text-sub">
                Page {summary.data.meta.page} of {summary.data.meta.totalPages}
              </p>
              <div className="flex gap-2">
                <Button
                  variant="outline"
                  disabled={page <= 1}
                  onClick={() => replaceQuery({ markPage: String(page - 1) })}
                >
                  Previous
                </Button>
                <Button
      ...
      <Dialog
        ...
        title={
          editingRow?.selectedDate?.exception
            ? `Edit ${editingRow.worker.name}`
            : `Mark ${editingRow?.worker.name ?? "worker"} absent`
        }
      ...
      >
        ...
        >
          {formError ? (
            <p
              role="alert"
              className="rounded-inner bg-danger/10 px-3 py-2 text-danger"
            >
              {formError}
            </p>
          ) : null}
          <label className="grid gap-1.5 font-semibold">
            ...
            <Select
              ...
              onChange={(event) =>
                setDraft((current) => ({
                  ...current,
                  duration: event.target.value as AttendanceDuration,
                }))
              }
            >
      ...
      <Dialog
        ...
        footer={
          <>
            ...
            <Button
              disabled={removeException.isPending || !canUpdate}
              onClick={() => void confirmRestore()}
            >
              {removeException.isPending ? "Removing" : "Remove absence"}
            </Button>
          </>
        }
      >
        ...
        <p>
          The absence exception will be removed. Attendance will follow the
          current calendar and effective assignment.
        </p>
      </Dialog>
    </div>
  );
          ...
          <TableBody>
            {rows.map((row) => (
              <TableRow key={row.workerAssignmentId}>
                <TableCell>
                  <WorkerHistoryLink row={row} projectId={projectId} />
...
function RowActions({
  ...
  if (!row.selectedDate)
    return <span className="text-sub">Date unavailable</span>;
  if (row.selectedDate.state === "NON_WORKING")
    return <span className="text-sub">No action</span>;
  ...
  return canMark ? (
    ...
    <span className="text-sub">Read only</span>
  );
}

function WorkerHistoryLink({
  row,
  projectId,
}: {
  row: AttendanceSummaryRow;
  ...
  return (
    <Link
      className="font-semibold underline decoration-hairline underline-offset-4"
      href={`/attendance?${next}`}
    >
      {row.worker.name}
    </Link>
  );
