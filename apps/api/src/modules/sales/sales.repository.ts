...
export class SalesRepository {
  ...
  async listUnits(
    ...
    return this.database.query<Row>(
      `SELECT u.id, u.unit_number unitNumber, u.unit_type unitType, u.wing_tower wingTower,
              u.floor, u.area_sqft areaSqft, u.facing, u.base_price basePrice, u.status,
  ...
  async createUnit(
    ...
    await this.database.execute(
      `INSERT INTO sales_units
        (id, organization_id, project_id, unit_number, unit_type, wing_tower, floor, area_sqft, facing, base_price, status, created_by, updated_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        organizationId,
  ...
  async updateUnit(
    ...
    const result = await this.database.execute(
      `UPDATE sales_units SET unit_number = ?, unit_type = ?, wing_tower = ?, floor = ?, area_sqft = ?,
         facing = ?, base_price = ?, status = ?, updated_by = ?
  ...
  async blockUnit(
    ...
    await this.database.transaction(async (connection) => {
      ...
      const leads = await this.database.query<Row>(
        `SELECT id FROM sales_leads WHERE id = ? AND organization_id = ? AND project_id = ? FOR UPDATE`,
      ...
      await this.database.execute(
        `INSERT INTO sales_unit_blocks
          (id, organization_id, project_id, unit_id, lead_id, blocked_by, expires_at, notes)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          id,
          organizationId,
  ...
  async releaseBlock(
    ...
    await this.database.transaction(async (connection) => {
      const rows = await this.database.query<Row>(
        `SELECT unit_id, lead_id, status FROM sales_unit_blocks
